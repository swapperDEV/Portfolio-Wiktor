...
import { projectList } from "../../assets/data/projects/projectList";
import { Fade } from "react-awesome-reveal";
import { skills } from "../../assets/data/projects/skills";
      ...
      <div className="skills_technology">
        <ul className="skills_list" data-aos="flip-up">
          {skills.map((skill) => (
            <li
              key={skill.id}
              onClick={() => changeActSkill(skill.id)}
              className={`skill_icon ${actSkill === skill.id && "choosed"}`}
            >
              <i>{skill.name()}</i>
            </li>
          ))}
