...
export const Navbar = (props) => {
  ...
  const linksFull = sections.map((e) => (
    <li
      key={e.name}
      className={`nav_item mobile_nav_item ${e.class}`}
      onClick={e.onClk}
    >
      {e.name} {e.icons}
    </li>
  ...
  return (
    <>
      ...
      <Wrapper className={navDisplay ? "navbar navbarUnHide" : "navbar"}>
        ...
        <Wrapper
          className={navDisplay ? "mobile_navbar navUnHide" : "mobile_navbar"}
        >
          <ul className="mobile_navbar_ul">{linksFull}</ul>
        </Wrapper>
      </Wrapper>
    </>
  );
